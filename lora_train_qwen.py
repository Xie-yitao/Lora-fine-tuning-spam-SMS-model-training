...
class QwenForSequenceClassification(transformers.PreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, labels=None, **kwargs):
        ...
        # 检查 outputs 是否包含 hidden_states
        if outputs.hidden_states is None:
            raise ValueError("Model did not return hidden_states. Please check the model configuration.")
        ...
        return SequenceClassifierOutput(
            loss=loss,
            logits=logits,
            hidden_states=None,  # 不传递 hidden_states 到输出
            attentions=None
        )
